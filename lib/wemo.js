...
class Wemo {
  ...
  addDiscoveredDevice (device) {
    ...
    let accessory
    if (device.deviceType === cns.deviceTypes.Bridge) {
      //* ** BRIDGED DEVICE ***\\
  ...
  addAccessory (device) {
    ...
    switch (device.deviceType) {
      case cns.deviceTypes.Insight:
      case cns.deviceTypes.Switch:
        serviceType = Service.Outlet
        break
      case cns.deviceTypes.LightSwitch:
        serviceType = Service.Switch
        break
      case cns.deviceTypes.Dimmer:
        serviceType = Service.Lightbulb
        break
      case cns.deviceTypes.Motion:
      case cns.deviceTypes.NetCamSensor:
        serviceType = Service.MotionSensor
        break
      case cns.deviceTypes.Maker:
        serviceType = Service.Switch
        break
      default:
  ...
  addLinkAccessory (link, device) {
    try {
      ...
      service.addCharacteristic(Characteristic.Brightness)
      if (device.capabilities[cns.linkAcc.temperature] !== undefined) {
