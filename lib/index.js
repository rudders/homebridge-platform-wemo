...
class WemoPlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      this.discoveryInterval = isNaN(this.discoveryInterval) || this.discoveryInterval < 15
        ? this.consts.defaults.discoveryInterval
      ...
      if (!this.funcs.hasProperty(this.ssdpOpts, 'explicitSocketBind')) {
  ...
  wemoSetup () {
    try {
      ...
      this.log('✓ Setup complete. %s', this.consts.logMessages[Math.floor(Math.random() * this.consts.logMessages.length)])
    ...
    this.manualDevices.forEach(async deviceURL => {
      try {
        ...
        if (deviceURL.includes(':')) {
          ...
          const port = await portScanner.findAPortInUse(this.consts.portsToScan, deviceURL)
