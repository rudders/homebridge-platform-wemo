...
class WemoPlatform {
  ...
  async loadDevice (setupUrl) {
    try {
      ...
      this.log('Loading %s with cbURL %s.', setupUrl, device.callbackURL)
      ...
      this.log.warn('Error loading device %s - %s.', setupUrl, errToShow)
    }
  }
  ...
  async initialiseDevice (device) {
    try {
      ...
      switch (device.deviceType) {
        ...
        default: {
          this.log.warn('[%s] is unsupported. Feel free to create a GitHub issue!', device.friendlyName)
          return
        }
      }
      if (!(
