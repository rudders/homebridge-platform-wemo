...
class WemoPlatform {
  ...
  wemoSetup () {
    try {
      ...
      if (!this.config.disableDiscovery) {
        this._ssdpClient = new SSDPClient(this.wemoClientOpts.discover_opts || { explicitSocketBind: true })
      }
  ...
  async loadDevice (setupUrl) {
    try {
      ...
      device.callbackURL = 'http://' + this.getLocalInterfaceAddress(location.hostname) + ':' + this._server.address().port
        ...
        **************************/
        ...
        if (device.deviceType === this.helpers.deviceTypes.Insight) {
          ...
          *****************/
          ...
          WEMO OUTLET
          **********/
          accessory.control = (this.config.outletAsSwitch || '').split(',').includes(device.serialNumber)
