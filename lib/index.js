  ...
  pluginSetup () {
    ...
    try {
      ...
      this.devicesInHB.forEach(accessory => {
        ...
        if (services && services[1] && typeof services[1] === 'object') {
          const service = services[1].constructor.name
          let charToError
          switch (service) {
            case 'AirPurifier':
            case 'HeaterCooler':
          ...
          if (service && charToError) {
            accessory.getService(this.api.hap.Service[service]).updateCharacteristic(
              this.api.hap.Characteristic[charToError],
              new this.api.hap.HapStatusError(this.consts.hapError)
            )
          }
        }
    ...
    if (this.config.mode === 'auto') {
      ...
      this.ssdpClient.on('response', async (msg, statusCode, rinfo) => {
        ...
        try {
          ...
          if (this.deviceClients[deviceUDN] && !this.deviceClients[deviceUDN].error) {
            return
          }
          ...
          if (!deviceData) {
            throw new Error('[' + deviceIP + '] ' + this.messages.noPort)
          }

          // Device doesn't have a valid client so sent it to the queue to load
          this.queue.add(async () => await this.initialiseDevice(deviceData))
        } catch (e2) {
        // Show warnings on runs 1, 2, 5, 8, 11, ... just to limit logging to an extent
          if (this.discoveryRun === 1 || this.discoveryRun % 3 === 2) {
            ...
            this.log.warn('[%s] %s: %s.', deviceIP, this.messages.connError, eText)
          }
        }
      })
      ...
      try {
        ...
        this.log.warn('%s %s.', this.messages.ssdpFail, eText)
      }
    }
    ...
    this.config.manualDevices.forEach(async device => {
      try {
        ...
        if (device.includes(':')) {
          ...
          const urlParts = new this.URL(device)
