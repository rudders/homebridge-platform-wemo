...
class WemoPlatform {
  constructor (log, config, api) {
    ...
    try {
      ...
      this.mode = ['auto', 'manual'].includes(this.config.mode) ? this.config.mode : 'auto'
    ...
    try {
      ...
      this.listenerServer = http.createServer((req, res) => {
        ...
        if (req.method === 'NOTIFY' && this.deviceClients[udn]) {
          ...
          if (this.debug) {
            this.log('[%s] incoming notification for unknown accessory.', udn)
          }
          res.writeHead(404)
          res.end()
        }
  ...
  discoverDevices () {
    this.discoveryRun++
    if (this.discoveryRun > 5 && Object.keys(this.devicesToDiscover).length === 0 && !this.forceDiscovery) {
      return
    }
    this.forceDiscovery = false
    this.log.error('Discovery Run #%s', this.discoveryRun)
    this.manualDevices.forEach(deviceURL => this.loadDevice(deviceURL, true))
    if (this.mode !== 'manual') {
    ...
    if (this.discoveryRun > 1) {
      for (const i in this.devicesToDiscover) {
        if (this.helpers.hasProperty(this.devicesToDiscover, i)) {
          this.log.warn('[%s] still awaiting (re)connection.', this.devicesToDiscover[i])
        }
      }
    }
  }
        ...
        case 'urn:Belkin:device:bridge:1': {
          try {
            ...
            subdevices.forEach(subdevice => {
              ...
              delete this.devicesToDiscover[uuidSub]
  ...
  addAccessory (device, isPri) {
    try {
      ...
      this.log.warn('[%s] could not be added to Homebridge as %s.', device.friendlyName, this.debug ? err : err.message)
    }
  }
