...
class WemoPlatform {
      ...
      switch (key) {
        ...
        case 'makerTypes':
          if (Array.isArray(val) && val.length > 0) {
            val.filter(x => x.serialNumber && x.serialNumber.length > 0)
              .forEach(x => {
        ...
        case 'mode':
          ...
          this.config.mode = val === 'manual' ? 'manual' : 'auto'
          break
        case 'name':
        case 'platform':
          break
  ...
  discoverDevices () {
    ...
    // Try and connect to the manually configured devices (if any)
    this.config.manualDevices.forEach(async deviceURL => {
      try {
        // Check to see if the entry is a full address or an IP
        if (deviceURL.includes(':')) {
          // It's a full address so sent straight to load
          this.loadDevice(deviceURL, true)
        } else {
          // It's an IP so perform a port scan
          const port = await this.portScanner.findAPortInUse(
            this.consts.portsToScan,
            deviceURL
          )

          // Found an open port so send it to load
        ...
        if (this.discoveryRun === 1 || this.discoveryRun % 3 === 2) {
          const eText = this.funcs.parseError(err)
          this.log.warn('[%s] %s: %s.', deviceURL, this.messages.connError, eText)
        }
      }
    })

    // Try and connect to the auto-discovered devices
    if (this.config.mode === 'auto') {
      ...
      this.ssdpClient.on('response', (msg, statusCode, rinfo) => {
        // Check it's a Wemo device (service type)
        if (msg.ST === 'urn:Belkin:service:basicevent:1') {
          // Send the discovered device to load
    ...
    // Log pending devices on every third discovery run to limit logging
  ...
  async loadDevice (setupUrl, manual) {
    ...
    try {
      ...
      // Don't continue if an auto discovery device is actually set up manually
      if (
        !manual &&
        (
          this.config.manualDevices.includes(setupUrl) ||
          this.config.manualDevices.includes(location.hostname)
        )
      ) {
        return
      }
