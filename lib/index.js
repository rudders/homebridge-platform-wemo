...
class WemoPlatform {
  ...
  discoverDevices () {
    if (this.discoveryRuns > 5 && Object.keys(this.devicesToDiscover).length === 0 && !this.forceDiscovery) {
      return
    }
    this.discoveryRun++
    ...
    if (this.discoveryRun % 3 === 2) {
      ...
      for (const i in this.devicesToDiscover) {
        if (this.helpers.hasProperty(this.devicesToDiscover, i)) {
          this.log.warn('[%s] still awaiting connection.', this.devicesToDiscover[i])
        }
      }
    }
