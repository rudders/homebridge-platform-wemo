...
class WemoPlatform {
  ...
  discoverDevices () {
    this.discoveryRun++
    if (this.discoveryRun > 5 && Object.keys(this.devicesToConnect).length === 0 && !this.forceDiscovery) {
      return
    }
    this.forceDiscovery = false
    if (this.debug) {
      this.log('Running device discovery run #%s...', this.discoveryRun)
    }
    ...
    if (this.discoveryRun > 1) {
      for (const i in this.devicesToConnect) {
        if (this.helpers.hasProperty(this.devicesToConnect, i)) {
          this.log.warn('[%s] still awaiting (re)connection.', this.devicesToConnect[i])
        }
      }
    }
  }
