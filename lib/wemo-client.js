...
const WemoClient = module.exports = function (config) {
  ...
  config.serviceList.service.forEach(service => {
...
WemoClient.prototype.getEndDevices = function (cb) {
  const parseDeviceInfo = function (data) {
    ...
    if (data.GroupID) {
      ...
      device.capabilities = mapCapabilities(data.GroupCapabilityIDs[0], data.GroupCapabilityValues[0])
    } else {
      // single device
      device.friendlyName = data.FriendlyName[0]
      device.deviceId = data.DeviceID[0]
      device.capabilities = mapCapabilities(data.CapabilityIDs[0], data.CurrentState[0])
    }
    // set device type
