...
module.exports = class wemoClient extends eventEmitter {
  constructor (debug, log, device) {
    ...
    device.serviceList.service.forEach(function (service) {
  ...
  soapAction (serviceType, action, body, cb) {
    ...
    this.request(options, payload, function (err, response) {
      if (err) {
        this.error = err.code
        this.emit('error', err)
        return cb(err)
  ...
  getEndDevices (cb) {
    var parseDeviceInfo = function (data) {
      ...
      // treat device group as it was a single device
        ...
        device.capabilities = this.mapCapabilities(
      ...
      // set device type
      if (Object.prototype.hasOwnProperty.call(device.capabilities, '10008')) {
        device.deviceType = 'dimmableLight'
      }
      if (Object.prototype.hasOwnProperty.call(device.capabilities, '10300')) {
  ...
  getDeviceStatus (deviceId, cb) {
    ...
    var parseResponse = function (err, data) {
      ...
      xml2js.parseString(data.DeviceStatusList, { explicitArray: false }, function (err, result) {
        ...
        var capabilities = self.mapCapabilities(deviceStatus.CapabilityID, deviceStatus.CapabilityValue)
  ...
  setLightColor (deviceId, red, green, blue, cb) {
    var color = this.rgb2xy(red, green, blue)
  ...
  setAttributes (attributes, cb) {
    ...
    var xmlAttributes = Object.keys(attributes).map(function (attributeKey) {
  ...
  _onListenerAdded (eventName) {
    ...
    var serviceType = EventServices[eventName]
  ...
  handleCallback (body) {
    ...
    xml2js.parseString(body, { explicitArray: false }, function (err, xml) {
      ...
      for (var prop in xml['e:propertyset']['e:property']) {
        if (Object.prototype.hasOwnProperty.call(handler, prop)) {
