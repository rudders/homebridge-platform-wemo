...
module.exports = class wemoClient extends eventEmitter {
  ...
  async soapAction (serviceType, action, body, cb) {
    try {
      ...
      return cb(null, response && response['s:Envelope']['s:Body']['u:' + action + 'Response'])
    } catch (err) {
      this.error = err.code
      this.emit('error', err)
      return cb(err)
  ...
  _onListenerAdded (eventName) {
    ...
    const serviceType = EventServices[eventName]
