...
module.exports = class wemoClient extends eventEmitter {
  constructor (debug, log, device) {
    ...
    this.on('newListener', (event, listener) => {
      const EventServices = {
        ...
        crockpotState: 'urn:Belkin:service:basicevent:1'
      }
  ...
  async soapAction (serviceType, action, body) {
    try {
      ...
      if (this.error) {
        throw new Error('Device ' + this.UDN + ' has reported error ' + this.error)
      }
  ...
  async handleCallback (body) {
    const handler = {
      ...
      crockpotState: async data => {
        this.emit('crockpotState', data)
        // for (const [k, v] of Object.entries(data)) {
        //  if (['mode', 'time', 'cookedTime'].includes(k)) {
        //    this.emit('crockpotState', k, v)
        //  }
        // }
      }
    }
