...
module.exports = class connectionUPNP extends eventEmitter {
  ...
  subscribe (serviceType) {
    try {
      ...
      req.on('error', err => {
        const errToShow = this.debug ? err : err.code + ': ' + err.message
        this.log.warn('[%s] subscription for service [%s] encountered error [%s].', this.device.friendlyName, serviceType, errToShow)
        this.subscriptions[serviceType] = null
        this.error = err.code
        this.emit('error', err)
      })
  ...
  async receiveRequest (body) {
    try {
      ...
      for (const prop in xml['e:propertyset']['e:property']) {
        if (this.helpers.hasProperty(xml['e:propertyset']['e:property'], prop)) {
          ...
          switch (prop) {
            case 'BinaryState':
              try {
                ...
                this.log.warn('[%s] failed to process BinaryState: %s.', this.device.friendlyName, eToShow)
              }
              break
            case 'Brightness':
              try {
                ...
                const eToShow = this.debug ? '\n' + e : e.message
                this.log.warn('[%s] failed to process Brightness: %s.', this.device.friendlyName, eToShow)
              }
              break
            case 'InsightParams': {
              try {
                ...
                const eToShow = this.debug ? '\n' + e : e.message
                this.log.warn('[%s] failed to process insightParams: %s.', this.device.friendlyName, eToShow)
              }
              break
            }
            case 'attributeList':
              try {
                ...
                const eToShow = this.debug ? '\n' + e : e.message
                this.log.warn('[%s] failed to process attributeList: %s.', this.device.friendlyName, eToShow)
              }
              break
            case 'StatusChange':
              try {
                ...
                const eToShow = this.debug ? '\n' + e : e.message
                this.log.warn('[%s] failed to process StatusChange: %s.', this.device.friendlyName, eToShow)
              }
              break
          }
        }
      ...
      this.log.warn('[%s] failed to process incoming message: %s.', this.device.friendlyName, errToShow)
    }
  }
}
