...
module.exports = class wemoPlatform {
  constructor (log, config, api) {
    ...
    this.superDebug = config.superDebug || false
    const wemoClientOpts = config.wemoClient || {}
    this._port = wemoClientOpts.port || 0
    this._listenInterface = wemoClientOpts.listen_interface
    this._clients = {}
    ...
    this._ssdpClient = new SSDPClient(wemoClientOpts.discover_opts || {})
      ...
      if (!err && json) {
        ...
        if (!this._clients[device.UDN] || this._clients[device.UDN].error) {
          if (this.debugLoadDevice) {
            ...
            this.log.error('---- platform:loadDevice ----')
  ...
  initialiseDevice (device) {
    try {
        ...
        **************************/
        try {
          deviceClient.getEndDevices((err, subdevices) => {
            ...
            if (!subdevices) {
              this.log.warn('An error occured getEndDevices() - subDevices not defined.')
              return
            }
      ...
      this.log('[%s] initialised from your network with mac address [%s].', device.friendlyName, device.macAddress)
    } catch (err) {
