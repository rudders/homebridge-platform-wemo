...
module.exports = class wemoPlatform {
    ...
    try {
      ...
      this._server = http.createServer((req, res) => {
        ...
        if ((req.method === 'NOTIFY') && this._clients[udn]) {
          ...
          req.on('end', () => {
            if (this.debug) this.log('Incoming Request for %s: %s', udn, body)
            this._clients[udn].handleCallback(body)
            res.writeHead(204)
            res.end()
          })
        } else {
          if (this.debug) this.log('Received request for unknown device: %s', udn)
          res.writeHead(404)
          res.end()
        }
      })
      if (this._listenInterface) {
        this._server.listen(this._port, this.getLocalInterfaceAddress(), err => {
          if (err) this.log.warn('Server Listen Error - %s', err)
        })
      } else {
      ...
      this.manualDevices.forEach(device => this.loadDevice(device))
      if (!this.config.disableDiscovery) {
        this.discoverDevices()
        setInterval(() => this.discoverDevices(), this.discoveryInterval * 1000)
      }
    } catch (err) {
      this.log.warn('********* Cannot load homebridge-platform-wemo *******')
  ...
  discoverDevices () {
    ...
    this._ssdpClient.on('response', (msg, statusCode, rinfo) => {
      if (msg.ST && msg.ST === 'urn:Belkin:service:basicevent:1') {
        // *** LOAD THE DEVICE *** \\
        this.loadDevice(msg.LOCATION)
        // *********************** \\
      }
    })
      ...
      (err, json) => {
        try {
          ...
          if (!this._callbackURL) {
            this._callbackURL = 'http://' + this.getLocalInterfaceAddress(location.hostname) + ':' + this._server.address().port
          }
          device.callbackURL = this._callbackURL
          if (!this._clients[device.UDN] || this._clients[device.UDN].error) {
            if (this.debugLoadDevice) {
              this.log.error('---- debugLoadDevice output ----')
              this.log.warn(JSON.stringify(device, null, 2))
              this.log.error('---- end debugLoadDevice output ----')
            }
          ...
          this.log.warn('An error occured loading a device - %s', err)
        }
      }
    )
  }
  ...
  initialiseDevice (device) {
    try {
      ...
      if (!this._clients[device.UDN] || this._clients[device.UDN].error) {
        this._clients[device.UDN] = new wemoClient(device)
      }
      ...
      if (device.deviceType === helpers.deviceTypes.Bridge) {
        ...
        **************************/
        try {
          deviceClient.getEndDevices((err, subdevices) => {
            if (err) {
            ...
            subdevices.forEach(subdevice => {
              ...
              this.log('[%s] initialised from your network with serial number [%s] and mac address [%s].', accessory.displayName, device.serialNumber, device.macAddress)
            })
          })
        } catch (err) {
        ...
        this.devicesInHB.set(uuid, accessory)
        this.log('[%s] initialised from your network with with serial number [%s] and mac address [%s].', accessory.displayName, device.serialNumber, device.macAddress)
      }
    } catch (err) {
  ...
  addAccessory (device, isPrimary) {
    try {
      ...
      this.log('[%s] added to Homebridge with serial number [%s] mac address and mac address [%s].', device.friendlyName, device.serialNumber, device.macAddress)
      return accessory
    } catch (err) {
