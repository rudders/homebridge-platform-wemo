...
const deviceHumidifier = require('./device/humidifier')
    ...
    if (this.errorLogFlag === 5) {
      ...
      for (const i in this.devicesToDiscover) {
        if (helpers.hasProperty(this.devicesToDiscover, i)) {
          this.log.warn('[%s] not discovered yet so cannot be controlled until found.', this.devicesToDiscover[i])
        }
      }
      for (const i in this.devicesWithError) {
        if (helpers.hasProperty(this.devicesWithError, i)) {
          this.log.warn('[%s] reported error so cannot be controlled until found again.', this.devicesWithError[i])
        }
      }
    }
    this.errorLogFlag++
        ...
        **************************/
        try {
          ...
          subdevices.forEach(subdevice => {
            ...
            this.log('[%s] discovered with serial number [%s] and mac address [%s].', accessory.displayName, device.serialNumber, device.macAddress)
            deviceClient.on('error', err => {
              this.devicesWithError[uuid] = accessory.displayName
        ...
        } else if (device.deviceType === helpers.deviceTypes.Humidifier) {
          /**************
          WEMO HUMIDIFIER
          **************/
          accessory.control = new deviceHumidifier(this, accessory, device)
        }
        this.devicesInHB.set(uuid, accessory)
        this.log('[%s] discovered with serial number [%s] and mac address [%s].', accessory.displayName, device.serialNumber, device.macAddress)
        deviceClient.on('error', err => {
