...
module.exports = class deviceMotion {
  constructor (platform, accessory, device) {
    ...
    this.noMotionTimer = parseInt(platform.wemoMotions[device.serialNumber])
    this.noMotionTimer = isNaN(this.noMotionTimer) || this.noMotionTimer < 0
      ? platform.noMotionTimer
      : this.noMotionTimer
    ...
    this.eveLastActivation = function () {
      self.C.call(this, 'Last Activation', 'E863F11A-079E-48FF-8F27-9C2605A29F52')
      this.setProps({
  ...
  externalUpdate (value) {
    try {
      const prevState = this.service.getCharacteristic(this.C.MotionDetected).value
      ...
      if (value || this.noMotionTimer === 0) {
        if (this.motionTimer) {
          if (!this.disableDeviceLogging) {
            this.log('[%s] noMotionTimer stopped.', this.dName)
          }
          clearTimeout(this.motionTimer)
          this.motionTimer = false
        }
        this.service.updateCharacteristic(this.C.MotionDetected, value)
        ...
        if (!this.disableDeviceLogging) {
          this.log('[%s] motion sensor [%s].', this.dName, value ? 'detected motion' : 'clear')
        }
      } else {
        if (!this.disableDeviceLogging) {
          this.log('[%s] noMotionTimer started [%d secs].', this.dName, this.noMotionTimer)
        }
        clearTimeout(this.motionTimer)
        this.motionTimer = setTimeout(() => {
          this.service.updateCharacteristic(this.C.MotionDetected, false)
          this.accessory.historyService.addEntry({
            time: Math.round(new Date().valueOf() / 1000),
            status: 0
          })
          if (!this.disableDeviceLogging) {
            this.log('[%s] motion sensor [clear] - noMotion timer completed.', this.dName)
          }
          this.motionTimer = false
      ...
      this.log.warn('[%s] motion sensor update [%s] error: %s.', this.dName, value, errToShow)
    }
  }
}
