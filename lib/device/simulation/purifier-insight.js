  ...
  async internalStateUpdate (value) {
    try {
      ...
      if (!value) {
        ...
        this.service.updateCharacteristic(this.hapChar.CurrentAirPurifierState, 1)
        ...
        // Log the change if appropriate
        if (this.enableLogging) {
          this.log('[%s] current purifying [no].', this.name)
  ...
  externalInUseUpdate (value) {
    try {
      ...
      if (value !== this.cacheInUse) {
        ...
        // Log the change if appropriate
        if (this.enableLogging) {
          this.log('[%s] current purifying [%s].', this.name, value ? 'yes' : 'no')
        }
      }
    } catch (err) {
      // Catch any errors
  ...
  externalConsumptionUpdate (power) {
    try {
      ...
      if (power !== this.cachePower) {
        ...
        // Log the change if appropriate
        if (this.enableLogging) {
          this.log('[%s] current consumption [%sW].', this.name, powerInWatts)
        }
      }
    } catch (err) {
      // Catch any errors
  ...
  externalTotalConsumptionUpdate (todayWm, todayOnSeconds) {
    try {
      if (todayWm !== this.accessory.context.cacheLastWM) {
        ...
        // Log the change
        this.log(
          '[%s] today ontime [%s] consumption [%s kWh] total consumption [%s kWh].',
          this.name,
          todayOnHours,
          todaykWh.toFixed(3),
