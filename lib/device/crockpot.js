...
module.exports = class deviceCrockpot {
  ...
  async getAttributes () {
    try {
      ...
      this.log.warn('----- Received update from device -----', this.accessory.displayName)
      this.log(
        'Received mode [%s (%s)] and time remaining [%s minutes].',
      ...
      this.log.warn('----- END Received update from device -----', this.accessory.displayName)
    } catch (err) {
  ...
  async setAttributes (mode, time) {
    this.log.warn('----- Sending update to device -----', this.accessory.displayName)
    this.log(
      'Sending mode [%s (%s)] and time remaining [%s minutes].',
      this.accessory.displayName,
      mode,
    ...
    this.log.warn('----- END Sending update to device -----', this.accessory.displayName)
  }
  ...
  async internalActiveUpdate (value, callback) {
    ...
    try {
      ...
      this.log.warn('----- "Active" Change via Home App -----', this.accessory.displayName)
      if (value === 0) {
      ...
      this.log.warn('----- END "Active" Change via Home App -----', this.accessory.displayName)
    } catch (err) {
      this.log.warn(
  ...
  async internalModeUpdate (value, callback) {
    ...
    try {
      ...
      this.log.warn('----- "Mode" Change via Home App -----', this.accessory.displayName)
      if ([0, 33].includes(value)) {
        ...
        this.service.updateCharacteristic(this.Characteristic.HeatingThresholdTemperature, 0)
        this.accessory.context.cacheTime = 0
        if (!this.disableDeviceLogging) this.log('[%s] setting cooking timer to [0 minutes] as OFF or WARM was selected.', this.accessory.displayName)
      }
      ...
      this.log.warn('----- END "Mode" Change via Home App -----', this.accessory.displayName)
    } catch (err) {
      this.log.warn(
  ...
  async internalCookingTimeUpdate (value, callback) {
    ...
    try {
      callback()
      if (value === prevState) return
      this.log.warn('----- "Cooking Time" Change via Home App -----', this.accessory.displayName)
      ...
      this.log.warn('----- END "Cooking Time" Change via Home App -----', this.accessory.displayName)
    } catch (err) {
      this.log.warn(
