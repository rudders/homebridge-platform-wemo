...
module.exports = class deviceCrockpot {
  ...
  async setAttributes (attributes) {
    ...
    const xmlAttributes = Object.keys(attributes)
      ...
      .join('')
    await this.client.soapAction('urn:Belkin:service:basicevent:1', 'SetCrockpotState', { attributeList: { '#text': xmlAttributes } })
  }
  ...
  async internalModeUpdate (value, callback) {
    ...
    try {
      ...
      const payload = { mode: newValue.toString() }
      if ([0, 33].includes(value)) {
        ...
        this.service.updateCharacteristic(this.Characteristic.HeatingThresholdTemperature, 0)
        payload.time = 0
        if (!this.disableDeviceLogging) this.log('[%s] setting cooking time to [0 minutes].', this.accessory.displayName)
      }
      if ([66, 99].includes(value) && this.service.getCharacteristic(this.Characteristic.CurrentTemperature).value === 0) {
        ...
        payload.time = 30
        if (!this.disableDeviceLogging) this.log('[%s] setting cooking time to [30 mins].', this.accessory.displayName)
      }
      ...
      await this.setAttributes(payload)
