  ...
  async internalCookingTimeUpdate (value, callback) {
    ...
    try {
      ...
      if (value !== 0 && [0, 33].includes(prevMode)) {
        ...
        if (!this.disableDeviceLogging) {
          this.log('[%s] setting mode to [low] since cooking time above zero and the last mode was OFF or WARM.', this.accessory.displayName)
        }
        modeChange = 51
    ...
    try {
      if (value !== this.accessory.context.cacheTime) {
        if (convertedValue > 0 && this.service.getCharacteristic(this.Characteristic.RotationSpeed).value === 0) {
          if (!this.disableDeviceLogging) {
            this.log(
              '[%s] updating fan speed to [33%] (WARM) as received a cooking time > 0, and previous state was OFF.', this.accessory.displayName
            )
          }
