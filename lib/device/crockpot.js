...
module.exports = class deviceCrockpot {
  ...
  async internalCookingTimeUpdate (value, callback) {
    // if the cooking time is changed and the mode is currently OFF or WARM, then set to LOW
  ...
  async internalModeUpdate (value, callback) {
    ...
    try {
      ...
      if ([66, 99].includes(value) && this.service.getCharacteristic(this.Characteristic.HeatingThresholdTemperature).value === 0) {
      ...
      await this.setAttributes(newValue, [0, 30].includes(setTime) ? setTime : null)
      if (!this.disableDeviceLogging) this.log('[%s] setting mode to [%s].', this.accessory.displayName, this.modeLabels[newValue])
      if ([0, 30].includes(setTime) && !this.disableDeviceLogging) {
        this.log('[%s] setting cooking time to [%s minutes].', this.accessory.displayName, setTime)
      }
    } catch (err) {
      this.log.warn(
