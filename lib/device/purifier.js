...
module.exports = class devicePurifier {
  constructor (platform, accessory, device) {
    ...
    this.airService = accessory.getService(this.Service.AirQualitySensor) || accessory.addService(this.Service.AirQualitySensor, 'Air Quality', 'airquality')
    this.ioService = accessory.getService(this.Service.Switch) || accessory.addService(this.Service.Switch, 'Ionizer', 'ionizer')
    this.service
  ...
  async getDeviceStatus () {
    try {
      ...
      if (this.debug) {
        this.log(
          '[%s] received update [Mode: %s], [AirQuality: %s], [Ionizer: %s].',
  ...
  async setDeviceStatus (attributes) {
  ...
  async internalModeUpdate (value, callback) {
    ...
    try {
      ...
      await this.setDeviceStatus({ Mode: newValue.toString() })
  ...
  async internalIonizerUpdate (value, callback) {
    ...
    try {
      ...
      await this.setDeviceStatus({ Ionizer: value ? '1' : '0' })
