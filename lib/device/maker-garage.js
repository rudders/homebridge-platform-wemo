...
module.exports = class deviceMakerGarage {
  ...
  async setDoorMoving (targetDoorState, homekitTriggered) {
        ...
        if (this.isMoving) {
          ...
          this.accessory.getService(this.Service.GarageDoorOpener).updateCharacteristic(this.Characteristic.CurrentDoorState, 4)
        ...
        if (homekitTriggered) {
          ...
          if (targetDoorState === this.Characteristic.TargetDoorState.CLOSED) {
            if (currentDoorState.value !== this.Characteristic.CurrentDoorState.CLOSED) {
              this.accessory.getService(this.Service.GarageDoorOpener).updateCharacteristic(this.Characteristic.CurrentDoorState, 3)
            ...
            if (
              ...
              (!this.sensorPresent && currentDoorState.value !== this.Characteristic.CurrentDoorState.OPEN)
            ) {
              this.accessory.getService(this.Service.GarageDoorOpener).updateCharacteristic(this.Characteristic.CurrentDoorState, 2)
        ...
        this.movingTimer = setTimeout(
          () => {
            delete this.movingTimer
            delete this.isMoving
            const targetDoorState = this.accessory.getService(this.Service.GarageDoorOpener).getCharacteristic(this.Characteristic.TargetDoorState).value
            if (!this.sensorPresent) {
              this.accessory.getService(this.Service.GarageDoorOpener).updateCharacteristic(this.Characteristic.CurrentDoorState, targetDoorState ? 1 : 0)
  ...
  externalSensorUpdate (state, wasTriggered) {
    ...
    const service = this.accessory.getService(this.Service.GarageDoorOpener)
    const targetDoorState = service.getCharacteristic(this.Characteristic.TargetDoorState).value
    const currentDoor = service.getCharacteristic(this.Characteristic.CurrentDoorState)
    if (targetDoorState === 0) {
      if (value === 0) {
        // Garage door's target state is OPEN and the garage door's current state is OPEN
        if (this.isMoving) {
          service.updateCharacteristic(this.Characteristic.CurrentDoorState, 2)
          if (this.debug) this.log('[%s] updated current state [opening].', this.accessory.displayName)
        } else {
          service.updateCharacteristic(this.Characteristic.CurrentDoorState, 0)
        ...
        delete this.isMoving
        service.updateCharacteristic(this.Characteristic.TargetDoorState, 1)
        service.updateCharacteristic(this.Characteristic.CurrentDoorState, 1)
      ...
      if (value === 1) {
        ...
        // it must have been triggered externally by a remote control
        service.updateCharacteristic(this.Characteristic.TargetDoorState, 0)
