...
module.exports = class deviceMakerGarage {
  constructor (platform, accessory, device) {
    ...
    if (!(this.service = this.accessory.getService(this.hapServ.GarageDoorOpener))) {
      ...
      this.service.addCharacteristic(this.eveChar.TimedOpened)
    }

    // Obtain the current times opened value
    this.timesOpened = this.service.getCharacteristic(this.eveChar.TimedOpened).value
    ...
    this.service.getCharacteristic(this.eveChar.ResetTotal).onSet(value => {
      this.timesOpened = 0
      this.service.updateCharacteristic(this.eveChar.TimedOpened, 0)
    })
    ...
    this.service.getCharacteristic(this.hapChar.TargetDoorState).onSet(async value => {
      await this.internalDoorUpdate(value)
    })
  ...
  async internalDoorUpdate (value) {
  ...
  externalDoorUpdate () {
    try {
      ...
      if (state === 0) {
        ...
        this.timesOpened++
        this.service.updateCharacteristic(this.eveChar.TimedOpened, this.timesOpened)
      }
    ...
    try {
      ...
      if (target === 0) {
        ...
        if (value === 0) {
          ...
          if (!this.disableDeviceLogging) {
            this.log(
              '[%s] current state [closed] (triggered externally).',
              this.name
            )
          }
        }
    ...
    this.movingTimer = setTimeout(
      () => {
        ...
        if (!this.sensorPresent) {
          ...
          if (!this.disableDeviceLogging) {
            const label = target === 1 ? 'closed' : 'open'
            this.log('[%s] current state [%s].', this.name, label)
          }
          return
        }
        if (target === 1) {
