...
module.exports = class deviceMakerGarage {
    ...
    if (targetDoorState === 0) {
      if (value === 0) {
        ...
        if (this.isMoving) {
          ...
          this.log('[%s] updated current state [opening].', this.accessory.displayName)
        } else {
          service.updateCharacteristic(this.Characteristic.CurrentDoorState, helpers.garageStates.Open)
          this.log('[%s] updated current state [open].', this.accessory.displayName)
        }
      } else {
      ...
      if (value === 1) {
        ...
        service.updateCharacteristic(this.Characteristic.CurrentDoorState, helpers.garageStates.Closed)
        this.log('[%s] updated current state [closed].', this.accessory.displayName)
      } else {
        ...
        this.log('[%s] setting target state [open] (triggered externally).', this.accessory.displayName)
  ...
  async setDoorMoving (targetDoorState, homekitTriggered) {
    ...
    if (this.isMoving) {
      ...
      this.log('[%s] updated current state [stopped].', this.accessory.displayName)
    ...
    if (homekitTriggered) {
      ...
      if (targetDoorState === helpers.garageStates.Closed) {
        if (currentDoorState !== helpers.garageStates.Closed) {
          ...
          this.log('[%s] updated current state [closing].', this.accessory.displayName)
        }
