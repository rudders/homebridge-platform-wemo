...
module.exports = class deviceMakerGarage {
  constructor (platform, accessory, device) {
    ...
    this.exposeContactSensor = deviceConf && deviceConf.exposeContactSensor
    ...
    // Set the garage door service as the primary service (ie not a contact sensor)
    ...
    try {
      ...
      try {
        ...
        if (this.exposeContactSensor) {
          ...
          this.accessory.eveService.addEntry({ status: prevCurrent === 0 ? 1 : 0 })
        }
      } catch (e) {}
    }
  }
  ...
  externalSensorUpdate (state, wasTriggered) {
    ...
    if (target === 0) {
      if (value === 0) {
        ...
        if (this.exposeContactSensor) {
          this.contactSensor.updateCharacteristic(this.hapChar.ContactSensorState, 1)
        ...
        if (this.exposeContactSensor) {
          this.contactSensor.updateCharacteristic(this.hapChar.ContactSensorState, 0)
    ...
    this.movingTimer = setTimeout(
      () => {
        ...
        if (!this.sensorPresent) {
          ...
          if (this.exposeContactSensor) {
            // target = 0 for open 1 for closed
            // we need to add 0 for contact detected and 1 for not
            this.contactSensor.updateCharacteristic(
              this.hapChar.ContactSensorState,
              target === 0 ? 1 : 0
            )
            this.accessory.eveService.addEntry({ status: target === 0 ? 1 : 0 })
            if (target === 0) {
              const initialTime = this.accessory.eveService.getInitialTime()
              this.contactService.updateCharacteristic(
                this.eveLastActivation,
