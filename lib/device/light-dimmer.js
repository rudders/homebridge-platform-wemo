...
module.exports = class deviceLightDimmer {
  constructor (platform, accessory, device) {
    ...
    if (!(service = accessory.getService(this.Service.Lightbulb))) {
      service = accessory.addService(this.Service.Lightbulb)
    ...
    accessory.client
      .on('error', err => this.log.warn('[%s] reported error - %s.', accessory.displayName, err.code))
      .on('binaryState', state => this.updateSwitchState(state))
      .on('brightness', newBrightness => this.updateBrightness(newBrightness))
