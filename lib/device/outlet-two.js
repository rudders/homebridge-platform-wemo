...
module.exports = class deviceSwitch {
  ...
  async internalUpdateA (value, callback) {
    let prevState
    try {
      prevState = this.serviceA.getCharacteristic(this.Characteristic.On).value
      callback()
      /*
      ...
      */
      if (!this.disableDeviceLogging) {
        this.log('[%s] setting outlet A state to [%s].', this.accessory.displayName, value ? 'on' : 'off')
      }
    } catch (err) {
      try {
        const errToShow = this.debug ? '\n' + err : err.message
        this.log.warn('[%s] setting outlet A state to [%s] error: %s.', this.accessory.displayName, value ? 'on' : 'off', errToShow)
        this.log.warn('[%s] reverting HomeKit status due to error.', this.accessory.displayName)
        await this.helpers.sleep(1000)
        this.serviceA.updateCharacteristic(this.Characteristic.On, prevState)
      } catch (e) {}
    }
  }
  ...
  async internalUpdateB (value, callback) {
    let prevState
    try {
      prevState = this.serviceB.getCharacteristic(this.Characteristic.On).value
      callback()
      /*
      await this.sendDeviceUpdate({
        BinaryState: value ? 1 : 0
      ...
      */
      if (!this.disableDeviceLogging) {
        this.log('[%s] setting outlet B state to [%s].', this.accessory.displayName, value ? 'on' : 'off')
      }
    } catch (err) {
      try {
        const errToShow = this.debug ? '\n' + err : err.message
        this.log.warn('[%s] setting outlet B state to [%s] error: %s.', this.accessory.displayName, value ? 'on' : 'off', errToShow)
        this.log.warn('[%s] reverting HomeKit status due to error.', this.accessory.displayName)
        await this.helpers.sleep(1000)
        this.serviceB.updateCharacteristic(this.Characteristic.On, prevState)
      } catch (e) {}
    }
  }
