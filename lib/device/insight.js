...
module.exports = class deviceInsight {
  constructor (platform, accessory, device) {
    ...
    this.client
      .on('binaryState', value => this.receiveDeviceUpdate('binaryState', value))
      .on('insightParams', (state, power, data) => this.receiveDeviceUpdate('insightParams', { state, power, data }))
  }
  ...
  receiveDeviceUpdate (attribute, value) {
    ...
    switch (attribute) {
      ...
      case 'brightness':
        this.externalInsightUpdate(parseInt(value.state), parseInt(value.power), value.data)
        break
    }
  }
  ...
  externalInsightUpdate (value, power, data) {
    ...
    this.externalOutletInUseUpdate(value !== 0)
  ...
  externalConsumptionUpdate (power) {
    ...
    try {
      ...
      const errToShow = this.debug ? '\n' + err : err.message
      this.log.warn('[%s] updating consumption to [%sW] error: %s.', this.accessory.displayName, consumption, errToShow)
    }
  }
