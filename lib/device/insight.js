...
module.exports = class deviceStandard {
  ...
  externalInsightUpdate (value, power, data) {
    this.log.warn('[%s] received insight data: state [%s] power [%sW]. Updating...', this.accessory.displayName, value, Math.round(power / 100))
    ...
    this.log.warn('[%s] update finished.', this.accessory.displayName)
  }
