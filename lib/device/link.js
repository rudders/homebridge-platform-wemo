...
module.exports = class deviceLink {
  ...
  async requestDeviceUpdate () {
    try {
      ...
      const result = await xml2js.parseStringPromise(data.DeviceStatusList, {
        explicitArray: false
      })
  ...
  async internalBrightnessUpdate (value) {
    try {
      const updateKeyBR = Math.random().toString(36).substr(2, 8)
      this.updateKeyBR = updateKeyBR
      await this.funcs.sleep(300)
      if (updateKeyBR !== this.updateKeyBR) {
        return
      }
      await this.sendDeviceUpdate(
