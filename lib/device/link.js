...
module.exports = class deviceLink {
  constructor (platform, accessory, link, device) {
    ...
    this.linkCodes = this.helpers.linkCodes
    ...
    this.device = device
    this.hasCTSupport = this.device.capabilities[this.linkCodes.temperature]
    ...
    // *** Add the set handler to the brightness characteristic *** \\
    this.service
      .getCharacteristic(this.Characteristic.Brightness)
      .removeAllListeners('set')
      .on('set', this.internalBrightnessUpdate.bind(this))

    // *** Add the set handler to the colour temperature characteristic *** \\
    if (this.hasCTSupport) {
      this.service
  ...
  receiveDeviceUpdate (attribute, value) {
    if (this.device.capabilities[attribute] === value) return
    this.device.capabilities[attribute] = value
    if (this.debug) {
  ...
  async internalSwitchUpdate (value, callback) {
    ...
    try {
      ...
      this.device.capabilities[this.linkCodes.switch] = value ? 1 : 0
  ...
  async internalBrightnessUpdate (value, callback) {
    ...
    try {
      ...
      this.device.capabilities[this.linkCodes.brightness] = value * 2.55
      if (!this.disableDeviceLogging) {
  ...
  async internalColourUpdate (value, callback) {
    ...
    try {
      ...
      this.device.capabilities[this.linkCodes.temperature] = value
      if (!this.disableDeviceLogging) {
