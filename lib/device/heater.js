...
module.exports = class deviceHeater {
  ...
  async getAttributes () {
    try {
      ...
      if (this.debug) {
        this.log(
          '[%s] status update received - mode [%s], currentTemp [%s], setTemp [%s].',
  ...
  async internalActiveUpdate (value, callback) {
    ...
    */
    ...
    try {
      ...
      this.ignoreNextTemp = true
      setTimeout(() => (this.ignoreNextTemp = false), 10000)
    } catch (err) {
      this.log.warn(
  ...
  async internalModeUpdate (value, callback) {
    ...
    try {
      ...
      await this.setAttributes({
        ...
        SetTemperature: this.accessory.context.cacheLastOnTemp
      })
  ...
  externalTargetTempUpdate (value) {
    if (value < 16) return
    if (this.ignoreNextTemp && value === 16) return
    if (value > 50) value = Math.round((value - 32) * 5 / 9)
