...
module.exports = class deviceHeater {
  ...
  receiveDeviceUpdate (attribute, value) {
    ...
    switch (attribute) {
      case 'Mode':
        ...
        break
      case 'Temperature':
        this.externalCurrentTempUpdate(hkValue)
        break
      case 'SetTemperature':
        this.externalTargetTempUpdate(hkValue)
        break
    }
  }
  ...
  async internalTargetTempUpdate (value, callback) {
    ...
    try {
      ...
      if (!this.disableDeviceLogging) {
        this.log('[%s] setting target temp to [%s°C].', this.accessory.displayName, value)
      }
    } catch (err) {
      try {
        const errToShow = this.debug ? '\n' + err : err.message
        this.log.warn('[%s] setting target temp to [%s°C] error: %s.', this.accessory.displayName, value, errToShow)
        this.log.warn('[%s] reverting HomeKit status due to error.', this.accessory.displayName)
        await this.helpers.sleep(1000)
  ...
  externalTargetTempUpdate (value) {
    try {
      ...
      if (tempState !== value) {
        ...
        if (!this.disableDeviceLogging) {
          this.log('[%s] updating target temp to [%s°C].', this.accessory.displayName, value)
        }
      }
      this.accessory.context.cacheLastOnTemp = value
    } catch (err) {
      const errToShow = this.debug ? '\n' + err : err.message
      this.log.warn('[%s] updating target temp to [%s°C] error: %s.', this.accessory.displayName, value, errToShow)
    }
  }
  ...
  externalCurrentTempUpdate (value) {
    try {
      ...
      if (tempState !== value) {
        ...
        if (!this.disableDeviceLogging) {
          this.log('[%s] updating current temp to [%s°C].', this.accessory.displayName, value)
        }
      }
    } catch (err) {
      const errToShow = this.debug ? '\n' + err : err.message
      this.log.warn('[%s] updating current temp to [%s°C] error: %s.', this.accessory.displayName, value, errToShow)
    }
  }
}
