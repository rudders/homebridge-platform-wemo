...
module.exports = class deviceOutlet {
  constructor (platform, accessory, device) {
    ...
    this.client.on('error', err => this.log.warn('[%s] reported error - %s.', this.accessory.displayName, this.debug ? err : err.message))
  ...
  internalOutletUpdate (value, callback) {
    try {
      ...
      this.client.setBinaryState(value ? 1 : 0, err => {
        ...
        this.accessory.getService(this.Service.Outlet).updateCharacteristic(this.Characteristic.OutletInUse, true)
        callback()
      })
    } catch (err) {
